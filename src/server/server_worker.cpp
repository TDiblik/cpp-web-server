...
ServerWorker::ServerWorker(uint16_t port, RequestHandler onHandled) : _socket_fd(-1), _port(port), _onHandled(onHandled), _kq_ident(-1) {
  ...
  this->_requests.resize(Server::ULIMIT);
}
...
void ServerWorker::accept_and_handle() {
